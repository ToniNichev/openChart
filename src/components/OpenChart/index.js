...
const OpenChart = (CanvasId, _config, ChartDataConnector) => {
  ...
  function _initDrawing(attachEvents) {
    ...
    if(attachEvents) {
      ...
      document.getElementById(drawingCanvas).bind('touchstart', function(event) {
        dragChart = true
        clientX = event.originalEvent.touches[0].clientX;
      });

      document.getElementById(drawingCanvas).bind('touchmove', function(event) {
        if(dragChart) {
